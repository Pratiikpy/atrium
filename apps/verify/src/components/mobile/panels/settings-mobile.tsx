...
export function SettingsMobile() {
  ...
  const display = address ? `${address.slice(0, 6)}...${address.slice(-4)}` : 'Not connected';
  const chainLabel = chain?.name ? chain.name.toLowerCase().replace(/\s+/g, '-') : 'arb-sepolia';
  ...
  return (
    <div className="md:hidden flex flex-col gap-4">
      ...
      <section className="rounded-2xl border border-mob-line bg-mob-bg-card px-4 py-4">
        ...
        <div className="mt-1.5 font-mono text-[18px] text-mob-ink">{display}</div>
      ...
      <MoreSection heading="Trust mode">
        <MoreRow href="/lantern"  label="Lantern dashboard" right="proofs ↗" icon={<EyeIcon />} />
        <MoreRow href="/security" label="Security model" right="↗" icon={<LockIcon />} />
        <MoreRow href="/docs"     label="Documentation" right="↗" icon={<BookIcon />} />
      </MoreSection>
...
function MoreSection({ heading, children }: { heading: string; children: React.ReactNode }) {
  return (
    <section>
...
function MoreRow({ href, label, right, icon }: { href: string; label: string; right: string; icon: React.ReactNode }) {
  return (
    <li>
      <Link
        href={href as any}
        className="flex items-center gap-3 border-b border-mob-hairline px-4 py-3.5 last:border-b-0 hover:bg-mob-bg-elev/50"
      >
...
const ShieldIcon = () => <Icon><path d="M3 6l5-3 5 3v3c0 2.4-2.2 4-5 4s-5-1.6-5-4V6z" /><path d="M6.4 8.2l1.2 1.2 2.6-2.6" /></Icon>;
const DocIcon    = () => <Icon><rect x="3" y="2.5" width="10" height="11" rx="1" /><path d="M5.5 5.5h5 M5.5 8h5 M5.5 10.5h3" /></Icon>;
const ClockIcon  = () => <Icon><circle cx="8" cy="8" r="6" /><path d="M8 5v3.5l2 1.5" /></Icon>;
const GearIcon   = () => <Icon><circle cx="8" cy="8" r="2" /><path d="M8 1.5v2 M8 12.5v2 M1.5 8h2 M12.5 8h2" /></Icon>;
const GuardIcon  = () => <Icon><circle cx="8" cy="6" r="2.4" /><path d="M3 14c0-2.6 2.2-4 5-4s5 1.4 5 4" /></Icon>;
const BellIcon   = () => <Icon><path d="M4 11V7.5a4 4 0 1 1 8 0V11l1 1.5H3L4 11z" /></Icon>;
const EyeIcon    = () => <Icon><path d="M1.5 8s2.5-4.5 6.5-4.5S14.5 8 14.5 8s-2.5 4.5-6.5 4.5S1.5 8 1.5 8z" /><circle cx="8" cy="8" r="2" /></Icon>;
const LockIcon   = () => <Icon><rect x="3.5" y="7" width="9" height="6.5" rx="1" /><path d="M5.5 7V5a2.5 2.5 0 015 0v2" /></Icon>;
const BookIcon   = () => <Icon><path d="M3 4h6a2 2 0 012 2v8H5a2 2 0 01-2-2V4z" /><path d="M3 12h8" /></Icon>;
