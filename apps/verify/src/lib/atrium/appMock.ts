// Atrium app — mock datasets for the desktop shell.
// Source of truth: .lovable/refs/atrium-app.jsx (designer build)

export type AppVenue = {
  id: string; name: string; short: string; type: string;
  collateral: number; asset: string; chain: string; chainShort: string;
  integration: "native" | "hybrid" | "bridged"; status: "live" | "pending";
};
...
export type AppPosition = {
  id: string; venue: string; instrument: string; side: "long" | "short" | "supply";
  size: string; notional: number; leverage: number; entry: number; mark: number;
  pnl24h: number; pnlPct: number; margin: number;
};

export const APP_POSITIONS: AppPosition[] = [
...
export type AppAgent = {
  id: string; handle: string; strategy: string; pnl7d: number; copiers: number;
  fee: number; status: "running" | "paused"; sharpe: number; aum: number; mandates: number;
};
...
export type AppMandate = {
  id: string; agent: string; status: "active" | "expired" | "revoked";
  cap: number; used: number; ttlDays: number; venues: string[]; created: string; txCount: number;
};

export const APP_MANDATES: AppMandate[] = [
...
export type AppActivity = {
  id: string; ts: string; kind: "agent" | "sigil" | "deposit" | "vigil" | "lantern" | "trade" | "transfer";
  desc: string; value: string; hash: string; ok: boolean;
};

export const APP_ACTIVITY: AppActivity[] = [
...
export const APP_TAX_LOTS = [
  { id: "l1", asset: "rTSLA",    event: "perp realised gain", date: "2026-05-12", proceeds: 84200, cost: 71800, gain: 12400 },
  { id: "l2", asset: "rAAPL",    event: "spot disposal",      date: "2026-04-29", proceeds: 42100, cost: 38900, gain:  3200 },
  { id: "l3", asset: "PT-stETH", event: "PT redemption",      date: "2026-04-12", proceeds: 12080, cost: 12000, gain:    80 },
  { id: "l4", asset: "USTB",     event: "interest realised",  date: "2026-04-01", proceeds:  1084, cost:     0, gain:  1084 },
];
...
export const fmtUsd = (n: number, opts: { compact?: boolean; decimals?: number; signed?: boolean } = {}) => {
