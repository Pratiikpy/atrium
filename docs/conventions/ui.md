...
`verify.atrium.fi` is the demo surface. Per PRD §26.1.
...
- Backup path: if `verify.atrium.fi` 404s on judge day, fall back to a pre recorded Loom and a QR to a mirror.
